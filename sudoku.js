        ...
        $this.createTable = function () {
          ...
          for (var row = 0; row < defaults.numOfRows; row++) {
            ...
            for (var col = 0; col < defaults.numOfCols; col++) {
              ...
              if (col == 2 || col == 5)
                defaults.domMatrix[row][col].addClass("sdk-border");
              //add columns to rows
          ...
          while (items > 0) {
            ...
            if (defaults.domMatrix[row][col].children().length == 0) {
              defaults.domMatrix[row][col].append(
                "<div class='sdk-solution'>" +
                  defaults.matrix[row][col] +
                  "</div>"
              );
          ...
          defaults.table.find(".sdk-col").click(function () {
            ...
            if ($(this).children().length == 0) {
              //select this
              defaults.domMatrix[$(this).attr("data-row")][
                $(this).attr("data-col")
              ].addClass("sdk-selected");
              defaults.selected =
                defaults.domMatrix[$(this).attr("data-row")][
                  $(this).attr("data-col")
                ];
              defaults.selectedSolution =
                defaults.matrix[$(this).attr("data-row")][
                  $(this).attr("data-col")
                ];
            } else {
              //add helper style
          ...
          for (var a in defaults.anwerTracker) {
            ...
            if (defaults.anwerTracker[a] > 0) {
              ...
              answerContainer.append(
                "<div class='sdk-btn sdk-no-show'>" + a + "</div>"
              );
            }
          }
